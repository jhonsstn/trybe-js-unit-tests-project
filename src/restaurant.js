...
const payOrder = () => {
  ...
  let priceToPay = 0;
  let foodList = restaurant.fetchMenu().food;
  let drinkList = restaurant.fetchMenu().drink;
  for (let index = 0; index < orderList.length; index += 1) {
    if (Object.keys(foodList).includes(orderList[index])) {
      priceToPay += foodList[orderList[index]];
    }
    if (Object.keys(drinkList).includes(orderList[index])) {
      priceToPay += drinkList[orderList[index]];
    }
  }
  return priceToPay + priceToPay * 0.1;
};
