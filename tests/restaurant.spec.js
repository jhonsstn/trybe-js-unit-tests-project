...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(typeof returnedObject.fetchMenu === 'function').toBe(true);
    ...
    const returnedFetchMenuKeys = Object.keys(returnedObject.fetchMenu());
    expect(returnedFetchMenuKeys).toEqual(objectKeys);
    ...
    // ```
    const returnedFetchMenu = returnedObject.fetchMenu();
    expect(returnedFetchMenu).toMatchObject(object);
    ...
    const orderArray = ['coxinha', 'agua', 'sashimi'];
    for (let index = 0; index < orderArray.length; index++) {
      returnedObject.order(orderArray[index]);
    }
    expect(returnedObject.consumption).toEqual(orderArray);
    ...
    // ```
    returnedObject.consumption = [];
    const repeatedOrderArray = ['coxinha', 'agua', 'coxinha'];
    for (let index = 0; index < repeatedOrderArray.length; index++) {
      returnedObject.order(repeatedOrderArray[index]);
    }
    expect(returnedObject.consumption).toEqual(repeatedOrderArray);
    ...
    // ```
    returnedObject.consumption = [];
    const toPayOrderArray = ['coxinha', 'agua', 'cerveja'];
    for (let index = 0; index < toPayOrderArray.length; index++) {
      returnedObject.order(toPayOrderArray[index]);
    }
    expect(returnedObject.pay()).toBeCloseTo(16.17);
